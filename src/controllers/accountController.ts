...
class AccountController implements IController {
  ...
  private validateToken = async (
    ...
    response
      .status(200)
      .send(new Response(undefined, { result: true }).getData());
  };
  private CreateSupportTicket = async (
    request: IRequestWithUser,
    response: express.Response,
    next: express.NextFunction
  ) => {
          ...
          await request.user
            ...
            .then(async (user: IUser) => {
              if (user) {
                response
                  .status(201)
                  .send(new Response('Created Ticket Successfully').getData());
              } else {
                next(new SomethingWentWrongException());
              }
            })
  ...
  private GetTickets = async (
    ...
    for (let i in supportTicketsIDs) {
      await supportTicketModel
        .findById(supportTicketsIDs[i], {
          description: 1,
          category: 1,
          date: 1,
        })
        .then((ticket: ISupportTicket) => {
  ...
  private getTickets = async (
    ...
    await userModel
      .findById(request.user._id, { supportTickets: 1 })
      .then(async (supportTickets) => {
        if (supportTickets) {
          let supportTicketsIDs = {
            ...supportTickets.toObject().supportTickets,
          };
          await this.GetTickets(supportTicketsIDs)
            .then((supportTickets: string[]) => {
          ...
          response
            .status(200)
            .send(
              new Response(undefined, { formatedSupportTickets }).getData()
            );
        } else {
          next(new SomethingWentWrongException());
        }
  ...
  private getAllSupportCategories = async (
    request: express.Request,
    response: express.Response,
    next: express.NextFunction
  ) => {
    await supportTicketCategoryModal
  ...
  private insertSupportCategory = async (
    request: express.Request,
    response: express.Response,
    next: express.NextFunction
  ) => {
    ...
    if (isPasswordMatching) {
      ...
      await User.save((err) => {
        if (err) {
          ...
          response
            .status(200)
            .send(new Response('Password Updated Successfully!').getData());
        }
      });
    } else {

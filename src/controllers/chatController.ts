...
class ChatController implements IController {
    ...
    private initializeRoutes() {
        ...
        this.router.post(`${this.path}/SendMessage`, authMiddleware, validationMiddleware(ChatDTO), this.addMessage);
        this.router.get(`${this.path}/GetMsgs`, authMiddleware, this.GetMsgs);

    }
    private GetMsgs = async (request: IRequestWithUser, response: express.Response, next: express.NextFunction) => {
        await this.getActiveRequest(request.user)
            .then(async (req: IRequest) => {
                if (req) {
                    await conversationModel.findById(req.conversation)
                        .then((conversation : IConversation) => {
                            if (conversation) {
                                response.status(200).send(new Response(undefined, { ...conversation.toObject().messages }).getData());
                        ...
                        .then((conversation : IConversation) => {
                            if (conversation) {
                                ...
                                conversation.save().then(() => {
                                    response.status(200).send(new Response('Message Sent Succesfully', undefined).getData());
                                })
                                    .catch(err => {
                                        next(new SomethingWentWrongException(err))
                                    })
